...
def test_softmax_n(input_data, expected_numerators):
    output_0_builtin = softmax(input_data, dim=-1)
    assert output_0_builtin.size() == input_data.size()
    for idx in range(2):
        for jdx in range(3):
            assert output_0_builtin[idx][jdx].item() == approx(expected_numerators[idx][jdx]/ 10)
    ...
    for idx in range(2):
        for jdx in range(3):
            assert output_1[idx][jdx].item() == approx(expected_numerators[idx][jdx] / 11)

    output_4 = softmax_n(input_data, 4)
    assert output_4.size() == input_data.size()
    for idx in range(2):
        for jdx in range(3):
            assert output_4[idx][jdx].item() == approx(expected_numerators[idx][jdx] / 14)


def test_softmax_1(input_data, expected_numerators):
