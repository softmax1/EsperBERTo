...
def test_softmax_n_with_padding(input_data, expected_numerators, expected_denominators):
    ...
    output_1 = softmax_n_with_padding(input_data, 1)
    ...
    output_4 = softmax_n_with_padding(input_data, 4)
...
def test_softmax_n(input_data, expected_numerators, expected_denominators):
    ...
    output_1 = softmax_n_shifted_zeros(input_data, 1)
    assert output_1.size() == input_data.size()
    for idx in range(2):
        for jdx in range(3):
            assert output_1[idx][jdx].item() == approx(expected_numerators[idx][jdx] / (expected_denominators[idx] + 1))

    output_4 = softmax_n_shifted_zeros(input_data, 4)
    assert output_4.size() == input_data.size()
    for idx in range(2):
        for jdx in range(3):
            assert output_4[idx][jdx].item() == approx(expected_numerators[idx][jdx] / (expected_denominators[idx] + 4))
